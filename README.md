...
The local permission smoke wrappers separately probe whether a real Codex/OpenCode ACP runtime emits a permission request that Orbital can expose and resolve. A `pass` result means the full primary-mediated approval path was exercised. A `permission_capability_gap` result means the secondary harness completed the task without emitting an ACP permission request, so Orbital had no real permission event to mediate; this is a runtime capability/configuration gap, not evidence that Orbital dropped a request. For Codex local-subscription profiles, permission behavior appears tied to the local Codex runtime setting: `Ask for Approval` can surface ACP `session/request_permission` events, while `Approve for me` can allow the secondary Codex runtime to approve internally before Orbital sees a request. For OpenCode, the default `opencode_acp_local` profile follows the user's OpenCode configuration, which is permissive by default; the `opencode_acp_local_ask` profile injects `OPENCODE_CONFIG_CONTENT` so bash/edit actions are configured to ask and can be mediated through ACP. Explicit OpenCode Zen ask profiles combine permission mediation with a pinned model, such as `opencode_acp_big_pickle_ask` for `opencode/big-pickle`; run that manual smoke with `ORBITAL_OPENCODE_ACP_PERMISSION_PROFILE=opencode_acp_big_pickle_ask`. Big Pickle is documented by OpenCode Zen as free for a limited time, with a free-period data-use caveat, so use it only for non-sensitive smoke tasks. Codex-as-primary controlling Codex-as-secondary is useful for validation, but it is not the target operating model; Orbital is primarily aimed at a high-capability primary harness delegating bounded work to smaller, cheaper, or otherwise specialized secondary harnesses.

ACP conformance fixtures under `tests/fixtures/acp_conformance/` replay deterministic fake ACP cases and scrubbed protocol excerpts for legacy Codex ACP, official Codex ACP, and OpenCode ACP. OpenCode has separate evidence for the default/user-configured profile and the ask-config permission profile, including failure-mode fixtures for denial, missing option IDs, and permission resolution errors. These fixtures are the support-tier gate: real profiles stay `experimental_acp` until fixture coverage and manual smoke evidence prove their claimed capabilities.
